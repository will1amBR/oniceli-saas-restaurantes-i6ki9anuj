onRecordAfterUpdateSuccess((e) => {
  ...
  var prevStatus = order.original().getString('status')
  var isStockDeducted = order.getBool('stock_deducted')

  // Executa baixa somente quando status vira 'ready' e ainda não foi descontado
  if (currentStatus !== 'ready' || isStockDeducted) {
    return e.next()
  }
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    // Também registra a venda em sales_data para histórico e relatórios de faturamento
    try {
